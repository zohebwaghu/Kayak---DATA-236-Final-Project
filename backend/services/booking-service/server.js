...
// Consume booking requests from Kafka (async pattern)
(async () => {
  try {
    const kafka = createKafkaClient('booking-service');
    kafkaRequestConsumer = await createConsumer(kafka, 'booking-request-group');
    await subscribeToTopics(kafkaRequestConsumer, [TOPICS.BOOKING_REQUEST], async (topic, event) => {
      try {
        const result = await processBookingRequest(event.data || {});
        if (kafkaProducer) {
          await publishEvent(kafkaProducer, TOPICS.BOOKING_EVENTS, result.bookingId, {
            eventType: EVENT_TYPES.BOOKING_CREATED,
            data: { ...result, correlationId: event.correlationId }
          });
        }
      } catch (err) {
        console.error('❌ booking.request processing failed:', err);
        if (kafkaProducer) {
          await publishEvent(kafkaProducer, TOPICS.BOOKING_EVENTS, event?.data?.listingId || 'booking', {
            eventType: EVENT_TYPES.BOOKING_FAILED,
            data: { error: err.message, correlationId: event?.correlationId }
          });
        }
      }
    });
  } catch (error) {
    console.error('❌ Failed to initialize booking-request consumer:', error);
  }
})();
...
/**
 * Core booking processor (reused by HTTP and Kafka)
 */
async function processBookingRequest(payload) {
  ...
  try {
    ...
    try {
      ...
      const { availableCount } = availability[0];
...
app.post('/api/v1/bookings', async (req, res) => {
  try {
    const result = await processBookingRequest(req.body);
